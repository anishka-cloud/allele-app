...
export const track = {
  ...
  quizBackClicked: (fromQuestion) =>
    trackEvent("quiz_back_clicked", { from_question: fromQuestion }),

  quizCompleted: (resultData) =>
    trackEvent("quiz_completed", resultData),

  resultRevealed: ({ season, oliveFlag, priority }) =>
    trackEvent("result_revealed", { season, olive_flag: !!oliveFlag, priority: priority || "full" }),
  ...
  emailCaptureViewed: (season) =>
    trackEvent("email_capture_viewed", { season }),

  emailSubmitted: (season) =>
    trackEvent("email_submitted", { season }),

  emailSkipped: (season) =>
    trackEvent("email_skipped", { season }),

  emailSubmitFailed: (season, errorCode) =>
    trackEvent("email_submit_failed", { season, error_code: errorCode }),

  paidReportCtaClicked: ({ season, placement }) =>
    trackEvent("paid_report_cta_clicked", { season, placement: placement || "results" }),

  dossierCtaClicked: (season) =>
    trackEvent("dossier_cta_clicked", { season }),
};
